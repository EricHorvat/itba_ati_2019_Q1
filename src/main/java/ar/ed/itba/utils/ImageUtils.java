...
public final class ImageUtils {
  ...
  public static int[] normalize(final int[] pixels, int width) {
	...
	public static void drawLine(final int[] image, final int width, Pair<Integer, Integer> min, Pair<Integer, Integer> max) {
		if (Math.abs(max.getKey() - min.getKey()) < Math.abs(max.getValue() - min.getValue())) {
			if (min.getValue() > max.getValue())
				drawLineLow(image, width, max.getValue(), max.getKey(), min.getValue(), min.getKey());
			else
				drawLineLow(image, width, min.getValue(), min.getKey(), max.getValue(), max.getKey());
		}
  		else {
			if (min.getKey() > max.getKey())
				drawLineHigh(image, width, max.getValue(), max.getKey(), min.getValue(), min.getKey());
    		else
				drawLineHigh(image, width, min.getValue(), min.getKey(), max.getValue(), max.getKey());
		}
	}

	public static void drawLineLow(final int[] image, final int width, final int x0, final int y0, final int x1, final int y1) {
		int dx = x1 - x0;
		int dy = y1 - y0;
		int yi = 1;
	...
	public static void drawLineHigh(final int[] image, final int width, final int x0, final int y0, final int x1, final int y1) {
		int dx = x1 - x0;
		int dy = y1 - y0;
		int xi = 1;
